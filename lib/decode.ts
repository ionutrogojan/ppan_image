...
export default class Decoder {
	private format_to_hex(source: string): string[] {
	...
	private hex_to_bin(source: string[]): string[] {
		// temporary array
		const bin_array = [];
		...
		for (let i = 0; i < source.length; i++) {
			...
			for (let t = 0; t < temp.length; t++) {
				...
				temp[t] = converter.hex_to_bin(temp[t])
			}
	...
	private preview_bin(source: string[]): string {
		// ascii pixel values
		const [ black, white ] = [ "██", "░░" ];
	...
	public async decode(path:string) {
		// read file to string
		const format = await Deno.readTextFile(path);
		// parse format to hexadecimal array
		const hexadecimal = this.format_to_hex(format);
		// convert hexadecimal values to binary array
		const binary = this.hex_to_bin(hexadecimal);
		// log preview to console
		console.log(this.preview_bin(binary));
	}
}